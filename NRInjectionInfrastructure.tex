...
LAL frame is performed by the NR injection infrastructure,
following conventions described in Sec.~\ref{sec:coordinates}.}
...
\texttt{SimIMRSEOBNRv2ChirpTimeSingleSpin()} to estimate the waveform length with an additional leverage of 10\%. If the NR waveforms are not long enough for a given total mass and starting frequency, the generation is aborted.  \red{[HP describe what happens then: Is an error generated?  Is an empty array returned?  Is a \emph{longer} array allocated and the calculation proceeds?]}  From the estimated length and the desired sampling rate, the discrete time series for the spline evaluation is
determined.\\
