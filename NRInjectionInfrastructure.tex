  ...
  \begin{enumerate}
    ...
    \item Metadata describing the simulation, and identifying the
      ...
      \item Optionally, additional information about the dynamics of
        the black holes.
  \end{enumerate}
When multiple \red{NR data sets} \sout{sets of waveform-modes} for the
identical physical configuration (e.g. during a convergence test, or
for different choices during GW extraction) are provided, then each \red{NR data set 
...
\begin{alltt}
import romSpline 
spline = romSpline.ReducedOrderSpline(\(\tGW\), \(A\sb{\ell,m}(\tGW) or \Phi\sb{\ell,m}(\tGW)\), verbose=False)
...
alignment is discussed on page~\ref{tGW-vs-tNR}. \sout{
