...
\harald{Note that the time coordinate $\tGW$ in the wave-zone is \emph{not}
the same time coordinate $t$ which is used below to give information
about the dynamics of the black holes (e.g. {\tt spin1x-vs-time}).
There is no unambiguous identification of these time-coordinates,
since $\tGW$ and $t$ are defined at different regions of the
space-time (far-zone vs. near-zone).  It is desirable to approximately
relate these two time-coordinates to each other, e.g. to facilitate
statements about the dynamical state of the binary at the time ``when
a certain GW-frequency is reached''.  Therefore, the dynamics time $t$
should be specified such that same values of $t$ and $t_{\rm GW}$
correspond approximately to the same state of the evolution.}

%%%%%%%%%%%%%%%
\subsection{Amplitude and phase spline compression}
\label{sec:spline}
%%%%%%%%%%%%%%%
Gravitational waveforms for LIGO data analysis purposes require uniform sampling in time for a given sampling frequency.
NR datasets, however, are commonly not uniformly sampled and if they are, the sampling interval $\Delta \tGW$ may not necessarily
...
Once all spline interpolants for a single NR simulations have been obtained, they have to be stored as individual H5-groups in the \emph{final HDF5 output file} with the following \emph{mandatory} group naming conventions:\\

\begin{tabular}{|p{2.5cm}|p{3.7cm}|p{8cm}|}
  \hline 
\textbf{amp\textunderscore l\#1\textunderscore m\#2} & romspline-HDF5-group & amplitude $A_{\ell m}(t_{\rm GW})$ \\
\textbf{phase\textunderscore  l\#1\textunderscore m\#2} & romspline-HDF5-group  & phase $\Phi_{\ell m}(t_{\rm GW})$
\\ \hline 
\end{tabular}
$ $\\ \\

Here (\#1, \#2) are placeholders for $(\ell, m)$, e.g., for $(\ell=2, m=-2)$ the group naming convention is phase\textunderscore l2\textunderscore m-2 and 
...
%%%%%%%%%%%%%%%
\subsection{Metadata and additional HDF5 groups}
\label{sec:meta}
%%%%%%%%%%%%%%%
...
Vectors must be represented by their Cartesian components in the NR frame (see Sec.~\ref{sec:NRconv}). This ccoordinate system is consistent with the one used in the decomposition in spherical harmonics (e.g. the polar axis of the spherical harmonics must point along the +z-axis). No requirements are placed on the orientation of the NR frame. For instance, the orbital unit separation vector 'nhat' and the Newtonian unit orbital angular momentum 'LNhat' need not point along specific coordinate axes. Note also that the spin-fields in the metadata 'spin\{1,2\}\{x,y,z\}' are given in the NR frame, and \emph{not} in the LAL frame. The rotation of the GW modes into the LAL frame is taken care of by the NR injection infrastructure, following the conventions described in Sec.~\ref{sec:coordinates}. 

Note: The time-coordinate of spins, positions, masses and their respective time-series will be associated with the apparent horizon, e.g. the NR coordinate time $t_\mathrm{NR}$. Such a time cannot unambiguously be identified w1
ith the retarded time $\tGW$ of the waveform modes. However, the submitter is asked to make a reasonable effort to have the same numerical values of $t_\mathrm{NR}$ and $\tGW$ to correspond to the same portion of the waveform, e.g. through choosing $\tGW = t_\mathrm{NR} - R_\mathrm{extraction}$, where $R_\mathrm{extraction}$ is a finite extraction radius.
\\
\newline
Three versions of .h5 files, specified by the attribute `Format', are compatible. All of them require the following metadata stored as attributes of the .h5 file: \\
...
\textbf{spin1x} & float & x-component of the dimensionless spin of the heavier object in the NR simulation frame.\\

\textbf{spin1y} & float & y-component of the dimensionless spin of the heavier object in the NR simulation frame.\\

\textbf{spin1z} & float & z-component of the dimensionless spin of the heavier object in the NR simulation frame.\\

\textbf{spin2x} & float & x-component of the dimensionless spin of the lighter object in the NR simulation frame.\\

\textbf{spin2y} & float & y-component of the dimensionless spin of the lighter object in the NR simulation frame.\\

\textbf{spin2z} & float & z-component of the dimensionless spin of the lighter object in the NR simulation frame.\\

\textbf{LNhatx} & float & x-component of the Newtonian orbital angular momentum unit vector in the NR simulation frame.\\

\textbf{LNhaty} & float & y-component of the Newtonian orbital angular momentum unit vector in the NR simulation frame.\\

\textbf{LNhatz} & float & z-component of the Newtonian orbital angular momentum unit vector in the NR simulation frame.\\

\textbf{nhatx} & float & x-component of the orbital separation unit vector in the NR simulation frame defined as the unit vector pointing from the lighter (mass2) to the heavier (mass1) object.\\

\textbf{nhaty} & float & y-component of the orbital separation unit vector in the NR simulation frame.\\

\textbf{nhatz} & float & z-component of the orbital separation unit vector in the NR simulation frame.\\

\textbf{Omega} & float & orbital frequency. \\

\textbf{eccentricity} & float & estimated eccentricity of the simulation.\\

\textbf{mean\textunderscore anomaly} & float & estimated mean anomaly (cf. Eq.~\ref{eq:anomaly}). For $\mathrm{eccentricity} \leq 10^{-3}$, it is allowed to set mean\textunderscore anomaly to 0. If the mean anomaly has not been computed, set mean\textunderscore anomaly to -1.\\
\textbf{PN\textunderscore approximant} & string & \emph{Only present for PN-NR hyrbid waveforms}: identifier of the inspiral approximant 
\end{longtable}
...
In addition to the H5 groups that store the waveform data (amplitudes and phases), a H5-group for storing auxiliary information is required: 
\begin{itemize}
...
\end{itemize}
%
All of the above attributes and H5-groups are required for file formats 1,2 and 3. Format 2 additionally requires the following H5-groups that contain time series data obtained via \texttt{romSpline} in the same way as the amplitudes and phases (see Sec.~\ref{sec:spline} for details):
\begin{itemize}
...
To construct the NR GW polarisations $h_+$ and $h_\times$ in the LAL wave-frame, first the splines for each NR amplitude and phase are first evaluated at the required sampling times and convolved with the spin-weighted spherical harmonics\begin{align}
\label{}
...
We also thank Kent Blackburn and James Healy for providing useful comments on the manuscript, and Geoffrey Lovelace and Deirdre Shoemaker for input into the metadata discussion.
