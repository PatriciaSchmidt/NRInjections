...
\label{sec:NRconv}
%%%%%%%%%%%%%%%%
In Numerical Relativity one solves for the complete space-time of the binary system. For GW data analysis
purposes one requires the gravitational-wave strain $h$ far from the source. The relevant numerical quantity
is the metric perturbation $h_{ij}$ as computed in the transverse-traceless (TT) gauge. 
...
Let $( \ExNR, \EyNR, \EzNR)$ be a Cartesian coordinate system in the \emph{wave-zone}, i.e. the zone far away from the binary where the GWs are extracted, and $\tGW$ the corresponding far-zone time coordinate. This Cartesian coordinate system is related to the polar coordinates $(r, \theta, \phi)$ by the standard means. In this coordinate system, henceforth referred to as the NR frame, the metric perturbation is commonly decomposed into \emph{modes}
...
Note that the time coordinate $\tGW$ in the wave-zone is \emph{not} the same time coordinate that tracks the dynamics of the black holes in the near-zone. 
\patricia{Move text about time from frame section to here.}
%%%%%%%%%%%%%%%
...
\end{alltt}
The output of \texttt{spline.write} contains all information needed for subsequent interpolation of the respective input time-series data and is composed of five datasets: (deg, tol, X, Y, errors). \\
...
\begin{itemize}
  \item Amplitude H5-group: \textbf{amp\textunderscore l\#1\textunderscore m\#2}
  \item Phase H5-group: \textbf{phase\textunderscore  l\#1\textunderscore m\#2},
\end{itemize}
where (\#1, \#2) are placeholders for $(\ell, m)$, e.g., for $(\ell=2, m=-2)$ the group naming convention is phase\textunderscore l2\textunderscore m-2 and 
amp\textunderscore l2\textunderscore m-2. 
\patricia{Is it clear that there is one .h5 file per NR simulation?}

In addition to the amplitude and phase groups, we highly recommend to also store the NR waveform times $\tGW$, i.e., the one-dimensional data array containing the original time-steps of the NR data, as a H5-dataset in the final HDF5 file. 
